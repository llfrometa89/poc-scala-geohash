...
import com.stuart.geohash.infrastructure.stdio.{CommandLineRunner, CommandOptionsKeyword => Keyword}
...
object CommandFactory {
  ...
  def make[F[_]: Sync](command: Commands[F]): CommandFactory[F] =
    new CommandFactory[F] {
      ...
      val availableCommands = Map(
        Keyword.`import`  -> command.importCommand,
        Keyword.help      -> command.helpCommand,
        Keyword.helpAlias -> command.helpCommand
      )
